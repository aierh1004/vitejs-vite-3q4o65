<template>
  <form @submit.prevent="submitForm">
    <div calss="form-control" :class="{ invalid: !firstName.isValid }">
      <label for="firstname">Firstname</label>
      <input type="text" id="firstname" v-model.trim="firstName.val" />
      <p v-if="!firstName.isValid">Firstname must not be empty.</p>
    </div>
    <div calss="form-control" :class="{ invalid: !lastName.isValid }">
      <label for="lastname">Lastname</label>
      <input type="text" id="lastname" v-model.trim="lastName.val" />
      <p v-if="!lastName.isValid">Lastname must not be empty.</p>
    </div>
    <div calss="form-control" :class="{ invalid: !description.isValid }">
    ...
    <div calss="form-control" :class="{ invalid: !rate.isValid }">
      <label for="rate">Hourly Rate</label>
      <input type="number" id="rate" v-model.number="rate.val" />
      <p v-if="!description.isValid">Rate must be greater than 0.</p>
    </div>
    <div calss="form-control" :class="{ invalid: !areas.isValid }">
      <h3>Areas of Exportise</h3>
      <div>
        <input
          type="checkbox"
          id="frontend"
